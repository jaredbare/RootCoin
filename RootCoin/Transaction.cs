...
namespace RootCoin
{
    class Transaction
    {
        ...
        public void SignTransaction(PrivateKey signingKey)
        {
            ...
            {
                throw new Exception("You can't sign for other wallets!");
            }
            string txHash = this.CalclulateHash();
            this.Signature = Ecdsa.sign(txHash, signingKey);
        }
        public string CalclulateHash()
        {
            string fromAddressDER = BitConverter.ToString(FromAddress.toDer()).Replace("-", "");
        ...
        public bool IsValid()
        {
            ...
            return Ecdsa.verify(this.CalclulateHash(), this.Signature, this.FromAddress);
        }
    }
}
